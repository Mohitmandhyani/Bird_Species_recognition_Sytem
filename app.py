...
st.sidebar.title('What is "Bird Species Recoginisation System"')
st.sidebar.write('''
"What Bird is That?" is a **CNN Image Classification Model** which identifies the type of bird in an image. 

It can accurately identify over many different types of birds!

It was developed using close to few photos of birds using for transfer learning and fine-tuning of a pre-trained Image Classification Model.
...
if pred_button:
    ...
    # Display the prediction without confidence
    st.success(f'Predicted Species: **{top_prediction_row["Common Name"].title()}**')  # removed confidence level
